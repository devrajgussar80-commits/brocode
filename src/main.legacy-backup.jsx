...
function CompanyLogo({ className = "", name = "Bajjaj Finance Digital Service" }) {
  return <div className={`company-logo ${className}`.trim()}><b>{name}</b></div>;
}
...
function TelegramJoinModal({ onClose, companyName }) {
  return <div className="backdrop telegram-join-backdrop" onMouseDown={onClose}>
    <div className="telegram-join-modal" role="dialog" aria-modal="true" aria-labelledby="telegram-join-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <a className="primary telegram-join-button" href="https://t.me/BajjajFinanceDigitalService" target="_blank" rel="noopener noreferrer">Join Telegram Channel</a>
...
function ReferralDetails({ referral, embedded = false }) {
  ...
  const shareReferral = async () => {
    if (!referral) return;
    const text = `Join Bajjaj Finance with my referral code ${referral.code}`;
    try {
      if (navigator.share) await navigator.share({ title: "Bajjaj Finance Referral", text, url: referralLink });
...
function AdminCompanySettings({ companyName, busy, onSave }) {
  const [name, setName] = useState(companyName || "Bajjaj Finance Digital Service");
  useEffect(() => setName(companyName || "Bajjaj Finance Digital Service"), [companyName]);
...
function AdminWelcomePopupSettings({ settings, busy, onSave }) {
  const popupSettings = {
    ...
    title: settings?.welcome_popup_title || "Welcome to Meta Money Liquid Funds",
...
function AdminDashboard() {
  ...
  const load = useCallback(async (adminToken = token, silent = false) => {
    ...
    try {
      ...
      if (pendingAdminCredential.current === adminToken && window.PasswordCredential && navigator.credentials?.store) {
        navigator.credentials.store(new window.PasswordCredential({ id: "admin", name: "Meta Money Admin", password: adminToken })).catch(() => {});
  ...
  if (!token || (!data && !busy)) {
    return <main className="admin-login">
      <div className="admin-login-card">
        ...
        <p className="admin-kicker">Bajjaj Finance Control</p>
  ...
  const sections = [
    ...
    ["image_studio", "Image Studio", 0],
    ["app_settings", "App Settings", 3],
  ];
  const searchedRows = (section === "crypto" ? data?.crypto_recharges || [] : section === "balance_control" ? data?.users || [] : section === "withdrawal_receipt_time" ? data?.withdrawals || [] : ["overview", "app_settings", "image_studio"].includes(section) ? [] : data?.[section] || []).filter((row) =>
  ...
  return <div className={`admin-shell ${adminTheme === "dark" ? "admin-dark" : ""}`}>
    <header className="admin-header">
      <div className="admin-brand-lockup"><div className="admin-brand-mark"><ShieldCheck /></div><div><p className="admin-kicker">{data?.settings?.company_name || "Bajjaj Finance Digital Service"} Control</p><h1>Admin Dashboard</h1><small><span className="admin-live-indicator"><i /> Live operations</span>Last updated {dateTime(data?.generated_at)}</small></div></div>
    ...
    <section className="admin-panel">
      ...
      {!["overview", "image_studio", "support_conversations", "withdrawal_receipt_time"].includes(section) ? <div className="admin-toolbar"><div><Search /><input value={query} onChange={(e) => { setQuery(e.target.value); setSelectedRequestIds([]); setBulkConfirm(null); }} placeholder="Search name, email, ID, UTR..." /></div><small>{rows.length} records</small></div> : null}
      ...
      <div className="admin-table-wrap">
        ...
        {section === "image_studio" ? <AdminImageStudio plans={data?.plan_catalog || []} busy={busy} onApplyToPlan={uploadPlanImage} /> : null}
        {section === "app_settings" ? <div className="admin-app-settings"><AdminRechargeSettings settings={data?.settings} busy={busy} onSave={saveRechargeSettings} /><AdminHomeBannerSettings settings={data?.settings} busy={busy} onUpload={uploadHomeBanner} onRemove={removeHomeBanner} /><AdminCompanySettings companyName={data?.settings?.company_name} busy={busy} onSave={saveCompanyName} /><AdminWelcomePopupSettings settings={data?.settings} busy={busy} onSave={saveWelcomePopup} /></div> : null}
        {!rows.length && !["overview", "visitors", "users", "crypto", "app_settings", "plan_catalog", "image_studio", "notifications", "support_conversations"].includes(section) ? <div className="admin-empty">No matching records found.</div> : null}
      </div>
    </section>
  </div>;
}
...
function App() {
  ...
  const navStyle = {
    ...
    background: "#fff",
    borderTop: "1px solid #dce3eb",
    boxShadow: "0 -10px 28px rgba(11,37,87,.16)",
  ...
  return (
    <div className="app public-demo">
        ...
        {tab === "support" ? (
          ...
          <>
            <header>
              ...
              <button className="icon-btn notification-bell" type="button" aria-label={`Notifications${unreadNotificationCount ? `, ${unreadNotificationCount} unread` : ""}`} onClick={() => setShowNotifications(true)}>
                <Bell />
      ...
      {showTelegramJoin ? <TelegramJoinModal onClose={() => setShowTelegramJoin(false)} companyName={companyName} /> : null}
...
createRoot(document.getElementById("root")).render(window.location.pathname.replace(/\/$/, "") === "/admin" ? <AdminDashboard /> : <App />);
